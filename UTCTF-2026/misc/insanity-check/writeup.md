...
## Osint Path
The challenge description tells us that the flag is in CTFd. However, this contest platform is written on CTFd, so we predicted that the flag can be somewhere on the contest page.

Trying to exploit the contest website, when we get access to `https://utctf.live/robots.txt`, there are 2 hidden `.html` file, which is `/2065467898.html` and `/3037802467.html`:
...
Next, open the 2 hidden `.html` files, both file return to 404 not found:
...
However, when checking the view source of 2 hidden files, we have found a suspicious number array:
...
This looks like a XOR-encrypted, so we have a script to decode:
```python
